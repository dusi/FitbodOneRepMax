...
final class OneRepMaxListModelTests: XCTestCase {
    ...
    func testOneRepMaxes() async throws {
        mockDataStore.mockResult = .success([
            Exercise(date: Date(timeIntervalSinceReferenceDate: 1), name: "Deadlift", reps: 3, sets: 1, weight: 315),
            Exercise(date: Date(timeIntervalSinceReferenceDate: 1), name: "Deadlift", reps: 3, sets: 1, weight: 315),
            Exercise(date: Date(timeIntervalSinceReferenceDate: 2), name: "Back Squat", reps: 6, sets: 1, weight: 245),
            Exercise(date: Date(timeIntervalSinceReferenceDate: 2), name: "Back Squat", reps: 6, sets: 1, weight: 245),
            Exercise(date: Date(timeIntervalSinceReferenceDate: 3), name: "Bench Press", reps: 4, sets: 1, weight: 45),
            Exercise(date: Date(timeIntervalSinceReferenceDate: 3), name: "Bench Press", reps: 2, sets: 1, weight: 225),
        ...
        XCTAssertEqual(oneRepMaxes[2].latestOneRepMax, 66.66666666666666)

        XCTAssertEqual(oneRepMaxes[3].name, "Deadlift")
        XCTAssertEqual(oneRepMaxes[3].exercises.count, 1)
        ...
        XCTAssertEqual(oneRepMaxes[3].latestOneRepMax, 333.5294117647059)
    }
}
